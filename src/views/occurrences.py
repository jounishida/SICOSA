...
def render_occurrence_detail(user: dict):
    ...
    with fc1:
        ...
        status = st.selectbox("Situação", [""] + STATUS_OPTIONS, format_func=lambda x: x or "Todos", key="detail_filter_status", label_visibility="collapsed")
    with fc2:
        st.markdown("**Prioridade**")
        priority = st.selectbox("Prioridade", [""] + PRIORITY_OPTIONS, format_func=lambda x: x or "Todas", key="detail_filter_priority", label_visibility="collapsed")
    with fc3:
        st.markdown("**Título da ocorrência**")
        search = st.text_input("Título da ocorrência", placeholder="Digite protocolo, título ou descrição", key="detail_filter_search", label_visibility="collapsed")
    ...
    with st.form("detail_update_form"):
        ...
        if role == "solicitante":
            note = st.text_area("Comentário ou atualização", height=120)
            st.caption("Solicitantes podem comentar e anexar evidências, mas não alteram criticidade nem status por esta atualização.")
        else:
            c1, c2 = st.columns(2)
    ...
    can_close = role in {"solicitante", "atendente", "administrador"} and row["status"] != "Encerrada"
    if can_close:
        st.markdown("### Encerramento")
        with st.form("close_occurrence_form"):
            close_label = "Justificativa para encerramento" if role == "solicitante" else "Solução adotada"
            closure_notes = st.text_area(close_label, height=100)
            close_submitted = st.form_submit_button("Encerrar ocorrência")
            if close_submitted:
                if not closure_notes.strip():
                    st.error("Informe uma justificativa antes de encerrar a ocorrência." if role == "solicitante" else "Descreva a solução adotada antes de encerrar a ocorrência.")
                else:
                    try:
